/*
 * Copyright 2024 Ananth Kunchaka <ananth.kunchaka@hp.com>
 *
...
static gboolean
fu_hpi_cfu_claim_interface(FuHpiCfuDevice *self, GError **error)
{
...
static gboolean
fu_hpi_cfu_release_interface(FuHpiCfuDevice *self, GError **error)
{
	g_autoptr(GError) error_local = NULL;
...
fu_hpi_cfu_start_entire_transaction(FuHpiCfuDevice *self, GError **error)
{
	...
	guint8 start_entire_transaction_buf[] = {
	    0x25,
	    0x00,
	    0x00,
	    0xff,
					     ...
					     GError **error)
{
	...
	g_debug("start_entire_transaction_accepted: total bytes received: %x",
		(guint)actual_length);
...
fu_hpi_cfu_send_offer_list_accepted(FuHpiCfuDevice *self, gint *status, GError **error)
{
	...
	*status = 0;
	...
	g_debug("send_offer_list_accepted: total bytes received: %x", (guint)actual_length);
	...
	/* success */
	if (buf[13] == 0x01) {
		g_debug("device_send_offer_list_accepted success.");
	} else {
		if (buf[13] == 0x02) {
			g_warning(
...
fu_hpi_cfu_send_offer_update_command(FuHpiCfuDevice *self, FuFirmware *fw_offer, GError **error)
{
	...
	if (!fu_memcpy_safe(st_req->data, st_req->len, 0x1, data, size, 0x0, 16, error))
		return FALSE;
					  ...
					  GError **error)
{
	...
	*reply = 0;
	...
	g_debug("firmware_update_offer_accepted: total bytes received: %x", (guint)actual_length);
	...
	if (buf[13] == 0x01) {
		g_debug("firmware_update_offer_accepted: success.");
	} else {
		if (buf[13] == 0x02) {
			g_debug("firmware_update_offer_accepted:reason: %s",
				fu_cfu_rr_code_to_string(buf[9]));
		} else {
			g_debug("firmware_update_offer_accepted:reason: %s buf[13] is "
				"not a reject.",
				fu_cfu_rr_code_to_string(buf[9]));
		}
	}
...
static gboolean
fu_hpi_cfu_read_content_ack(FuHpiCfuDevice *self,
			    gint *lastpacket,
			    gint *report_id,
			    gint *reason,
			    gint *status,
			    GError **error)
{
	...
	*status = 0;
	...
	g_debug("read_content_ack at sequence_number:%d", self->sequence_number);
	if (!fu_usb_device_interrupt_transfer(FU_USB_DEVICE(self),
					      END_POINT_ADDRESS,
					      buf,
					      sizeof(buf),
	...
	g_debug("read_content_ack: bytes_received:%x", (guint)actual_length);
...
fu_hpi_cfu_firmware_update_offer_rejected(gint8 reply)
{
	...
	if (reply == FU_HPI_CFU_STATE_UPDATE_OFFER_REJECTED) {
		g_debug("firmware_update_offer_rejected: "
...
fu_hpi_cfu_send_end_offer_list(FuHpiCfuDevice *self, GError **error)
{
	...
	guint8 end_offer_list_buf[] = {0x25,
				       ...
				       0x00,
				       0x00};
	g_debug("send_end_offer_list sending:");
...
fu_hpi_cfu_end_offer_list_accepted(FuHpiCfuDevice *self, GError **error)
{
	...
	g_debug("end_offer_list_accepted: bytes_received:%x", (guint)actual_length);
	end_offer_response = g_bytes_new(buf, actual_length);
	fu_dump_bytes(G_LOG_DOMAIN,
		      "fu_hpi_cfu_end_offer_list_accepted: bytes received",
		      end_offer_response);

	g_debug("end_offer_list_accepted: Identify Type buffer[4]:%02x", (guchar)buf[4]);
	g_debug("end_offer_list_accepted: reject reason:buffer[9]:%02x, this value is "
		"meaningful when buffer[13]=2",
		(guchar)buf[9]);
	g_debug("end_offer_list_accepted: reply status:buffer[13]:%02x, response:%s",
		(guchar)buf[13],
				       ...
				       GError **error)
{
	...
	if (!fu_hpi_cfu_firmware_update_offer_accepted(self, &reply, error)) {
		self->state = FU_HPI_CFU_STATE_ERROR;
		g_prefix_error(error, "send_offer_accept: ");
		return FALSE;
	}
	if (reply == FU_HPI_CFU_FIRMWARE_UPDATE_OFFER_ACCEPT) {
		g_debug("firmware_update_offer_accepted: reply:%d, offer accepted", reply);
		...
		if (reply == FU_HPI_CFU_FIRMWARE_UPDATE_OFFER_SKIP) {
			...
			g_debug("firmware_update_offer_accepted: reply:%d, OFFER_BUSY", reply);
			self->retry_attempts++;
			...
			GError **error)
{
	...
	self->bytes_sent += cfu_buf->len;
	self->bytes_remaining = self->payload_file_size - (self->bytes_sent + 5);
					...
					GError **error)
{
	...
	if (self->last_packet_sent) {
		g_debug("handler_check_update_content: last_packet_sent");
		if (!fu_hpi_cfu_read_content_ack(self,
						 &lastpacket,
						 &report_id,
						 &reason,
						 &status,
	...
	if (status < 0) {
		g_debug("handler_check_update_content: FU_HPI_CFU_STATE_ERROR");
		self->state = FU_HPI_CFU_STATE_ERROR;
	} else {
		if (report_id == 0x25) {
			g_debug("handler_check_update_content: report_id:%d",
				report_id == FIRMWARE_REPORT_ID);
			switch (status) {
			case FU_HPI_CFU_FIRMWARE_UPDATE_OFFER_SKIP:
				g_debug("handler_check_update_content: OFFER_SKIPPED");
				self->state = FU_HPI_CFU_STATE_UPDATE_MORE_OFFERS;
				break;

			case FU_HPI_CFU_FIRMWARE_UPDATE_OFFER_ACCEPT:
				g_debug("handler_check_update_content: OFFER_ACCEPTED");
				if (lastpacket) {
					g_debug("handler_check_update_content: "
			...
			g_debug("handler_check_update_content: report_id:0x22");
			...
			case FU_HPI_FIRMWARE_UPDATE_STATUS_SUCCESS:
				g_debug("handler_check_update_content: SUCCESS");
				if (lastpacket) {
					self->state = FU_HPI_CFU_STATE_UPDATE_SUCCESS;
				} else
					self->state = FU_HPI_CFU_STATE_UPDATE_CONTENT;
...
static gboolean
fu_hpi_cfu_handler_send_payload_chunk(FuHpiCfuDevice *self,
				      FuChunk *chk,
				      FuProgress *progress,
				      void *options,
				      GError **error)
	...
	for (guint32 read_index = 0; read_index < payload_buf->len;) {
		...
		if (untransmitted_data->data != NULL) {
			if (untransmitted_data->len >= FU_HPI_CFU_PAYLOAD_LENGTH) {
				...
				if (!fu_hpi_cfu_send_payload(self, cfu_data, error))
					return FALSE;
				...
				if (remaining_byte_count) {
					...
					if (!fu_hpi_cfu_untransmitted_data(untransmitted_data,
				...
				if (remaining_byte_count >= FU_HPI_CFU_PAYLOAD_LENGTH) {
					...
					remaining_byte_count =
					    remaining_byte_count - FU_HPI_CFU_PAYLOAD_LENGTH;
					fill_from_position =
			...
			self->last_packet_sent = (read_index >= payload_buf->len) ? 1 : 0;
		} else {
		...
		if (self->last_packet_sent) {
			if (untransmitted_data->data != NULL) {
				...
				if (!fu_hpi_cfu_send_payload(self, cfu_last_packet, error))
					return FALSE;
			}
		}

		if (!fu_hpi_cfu_handler_check_update_content(self, progress, options, error))
			return FALSE;
...
fu_hpi_cfu_handler_send_payload(FuHpiCfuDevice *self,
				...
				GError **error)
{
	FuHpiCfuHandlerOptions *update_options = (FuHpiCfuHandlerOptions *)options;
	...
	for (guint32 i = 0; i < chunks->len; i++) {
		...
		if (!fu_hpi_cfu_handler_send_payload_chunk(self, chk, progress, options, error)) {
							 ...
							 GError **error)
{
	...
	if (reply == FU_HPI_CFU_FIRMWARE_UPDATE_OFFER_ACCEPT) {
		g_debug("handler_swap_pending_send_offer_list_accepted: "
		...
		if (fu_hpi_cfu_firmware_update_offer_rejected(reply)) {
			...
			case FU_HPI_CFU_FIRMWARE_OFFER_REJECT_MISMATCH:
				g_debug("handler_swap_pending_send_offer_list_accepted: "
					"FU_HPI_CFU_FIRMWARE_OFFER_REJECT_MISMATCH: expected a "
					"reject with SWAP PENDING");
				break;

			case FU_HPI_CFU_FIRMWARE_OFFER_REJECT_BANK:
				g_debug("FU_HPI_CFU_FIRMWARE_OFFER_REJECT_BANK: expected a reject "
					"with SWAP PENDING");
				break;

			case FU_HPI_CFU_FIRMWARE_OFFER_REJECT_PLATFORM:
				g_debug("FU_HPI_CFU_FIRMWARE_OFFER_REJECT_PLATFORM: expected a "
					"reject with SWAP PENDING");
				break;
			case FU_HPI_CFU_FIRMWARE_OFFER_REJECT_MILESTONE:
				g_debug("FU_HPI_CFU_FIRMWARE_OFFER_REJECT_MILESTONE: expected a "
					"reject with SWAP PENDING");
				break;

			case FU_HPI_CFU_FIRMWARE_OFFER_REJECT_INV_PCOL_REV:
				g_debug("FU_HPI_CFU_FIRMWARE_OFFER_REJECT_INV_PCOL_REV: expected a "
					"reject with SWAP PENDING");
				self->state =
			...
			case FU_HPI_CFU_FIRMWARE_OFFER_REJECT_VARIANT:
				g_debug("FU_HPI_CFU_FIRMWARE_OFFER_REJECT_VARIANT: expected a "
					"reject with SWAP PENDING");
				break;
...
static gboolean
fu_hpi_cfu_handler_swap_pending_send_end_offer_list(FuHpiCfuDevice *self,
				       FuProgress *progress,
				       void *options,
				       GError **error)
{
...
static gboolean
fu_hpi_cfu_handler_swap_pending_end_offer_list_accepted(FuHpiCfuDevice *self,
						FuProgress *progress,
						void *options,
						GError **error)
{
...
FuHpiCfuStateMachineFramework hpi_cfu_states[] = {
    ...
    {FU_HPI_CFU_STATE_END_OFFER_LIST, fu_hpi_cfu_handler_send_end_offer_list, NULL},
    ...
    {FU_HPI_CFU_STATE_VERIFY_CHECK_SWAP_PENDING_OFFER_ACCEPTED,
     ...
     NULL},
    {FU_HPI_CFU_STATE_VERIFY_CHECK_SWAP_PENDING_SEND_UPDATE_END_OFFER_LIST,
     fu_hpi_cfu_handler_swap_pending_send_end_offer_list,
     NULL},
    {FU_HPI_CFU_STATE_VERIFY_CHECK_SWAP_PENDING_UPDATE_END_OFFER_LIST_ACCEPTED,
     fu_hpi_cfu_handler_swap_pending_end_offer_list_accepted,
     NULL},
...
fu_hpi_cfu_device_setup(FuDevice *device, GError **error)
{
	...
	guint32 version_raw;
	gint32 version_table_offset = 4;
	gint32 component_id_offset = 5;
	gint32 component_data_size = 8;
	gint32 component_index = 0; /* multiple offers logic is in progress. */
	gint32 bulk_opt_index = 0;
	gsize actual_length = 0;
	guint8 buf[60] = {0};
	...
	bulk_opt_index = version_table_offset + component_index * component_data_size + component_id_offset;

	/* Get Bulk optimization value */
	if (!fu_memcpy_safe((guint8 *)&self->bulk_acksize,
			    sizeof(self->bulk_acksize),
			    0x0,
			    ...
			    error))
		return FALSE;
	g_debug("bulk_acksize: %d", self->bulk_acksize);
				 ...
				 GError **error)
{
	...
	if(!fu_hpi_cfu_claim_interface(self, error)) {
		g_prefix_error(error, "interface: ");
		return FALSE;
	}
	...
	if(!fu_hpi_cfu_release_interface(self, error)) {
		g_prefix_error(error, "interface: ");
		return FALSE;
	}
